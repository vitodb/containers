...
if [ ! "$(ls -A "GWMS_SRC_DIR")" ]; then
    git clone https://github.com/glideinWMS/glideinwms.git "$GWMS_SRC_DIR"
fi

[ ! "$(ls -A "GWMS_SRC_DIR")" ] && { echo "Failed to clone GlideinWMS Git repository. Unable to link it."; exit 1; } || true
...
pushd /usr/sbin || { echo "ERROR: /usr/sbin not found"; exit 1; }
...
pushd /usr/bin || { echo "ERROR: /usr/bin not found"; exit 1; }
